...
import numpy as np
from . import genemania as genemania
...
def run(run_obj):
    ...
    *terms_to_run*: terms for which to run the method. 
    ...
    for term in run_obj.terms_to_run:
        ...
        ## TEST try scaling between 0 and 1
        # This didn't make a difference
        #nonzero_idx = scores.nonzero()[0]
        #scores[nonzero_idx] = (scores[nonzero_idx] +1)/(1. + 1.)

        ## TEST check if there are actually values == k that are in a spearate connected component, and check if they are at the front of the smin curve(?)
        # by storing the nodes == k in each run, and then checking in the final evaluation.
        # num_pos = len(np.where(y == 1)[0])
        # num_neg = len(np.where(y == -1)[0])
        # k = float("%0.6f" % ((num_pos - num_neg) / float(num_pos + num_neg)))
        # rounded_scores = scores.round(6)
        # k_nodes = np.where(rounded_scores == k)[0]
        # print("%s nodes == k (k=%s)" % (len(k_nodes), k))
